/**
 * Commodity Service — Server-side data generation and access.
 */
import { roundToMaxDecimals, DISPLAY_MAX_DECIMALS } from '../utils/helpers';

interface CommodityPricePoint { date: string; price: number; volume: number; }

const generateCommodityHistory = (basePrice: number, baseVolume: number, days: number = 30): CommodityPricePoint[] => {
    ...
    for (let i = days; i >= 0; i--) {
        ...
        currentVolume += (Math.random() - 0.5) * currentVolume * 0.1;
        history.push({
...
const generateCommodities = () => {
    return COMMODITY_CONFIGS.map(config => {
        ...
        return {
            ...
            dayHigh: Math.max(...prices.slice(-24)),
            dayLow: Math.min(...prices.slice(-24)),
            history
        };
    });
};

let cachedCommodities: any[] | null = null;

export const getAllCommodities = () => {
    if (!cachedCommodities) cachedCommodities = generateCommodities();
    return cachedCommodities;
};

export const getCommodityById = (id: string) => getAllCommodities().find((c: any) => c.id === id);
